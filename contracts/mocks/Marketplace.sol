...
contract Marketplace {
    ...
    event NewTrade(
        ...
        address indexed traderOne,
        address indexed traderTwo,
        uint amount,
        uint price,
        uint date
        );
    ...
    function createLimitOrder(uint amount, bytes32 ticker, uint price, Side side) tokenExist(ticker) tokenIsNotDai(ticker)  external {
        if (side == Side.SELL) {
